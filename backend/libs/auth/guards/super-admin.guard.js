...
let SuperAdminGuard = class SuperAdminGuard {
    canActivate(context) {
        ...
        if (user.role !== client_1.UserRole.SUPER_ADMIN || user.scope !== client_1.UserScope.SYSTEM) {
            throw new common_1.UnauthorizedException('Access denied: SUPER_ADMIN privileges required');
        }
        return true;
    }
};
